...
function BookmarkCategoryMenu(props) {
    ...
    return (
        <ul className={`${styles.actions} ${props.openMenuId == props.category.id ? styles.open : ''}`}>
            <li key={props.category.id + 'add'} className={styles.action} onClick={props.toggleAddBookmarkFormVisibility}>
                <Svg content={<><path d="M5 12h14"/><path d="M12 5v14"/></>}/>
                <span className={styles.add}>Add bookmark</span>
            </li>
            <li key={props.category.id + 'edit'} className={styles.action} onClick={props.toggleEditBookmarkCategoryFormVisibility}>
            ...
            {props.isBookmarkCategoryMarkedForDeletion ? (
            <li key={props.category.id + 'confirm'} className={styles.action} onClick={handleConfirmBookmarkCategoryDeletion}>
            ...
            <li key={props.category.id + 'delete'} className={styles.action} onClick={props.markBookmarkCategoryForDeletion} data-delete-action>
